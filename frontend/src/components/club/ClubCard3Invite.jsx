    ...
    <>
      <div className="space-y-3">
        {clubList.map((item) => (
          <div
            key={item._id}
            className="group relative bg-white rounded-2xl shadow-sm hover:shadow-md overflow-hidden flex cursor-pointer transition-all duration-300 border border-gray-100 hover:border-primary-100"
          ...
          >
            ...
            <div className="flex flex-col flex-1 p-3 min-w-0 pr-[70px]">
              {item.admin === user.email && (
                <span className="self-start mb-1 bg-primary-600 text-white text-[10px] px-2 py-0.5 rounded-full font-nanum-bold">
                  내가 만든 모임
                </span>
              )}
              ...
              <div className="flex items-center gap-2 mt-auto">
                <div className="flex -space-x-2">
                  {item.members?.slice(0, 4).map((m, i) => (
                    <img
                      key={i}
                      src={m.img || "https://via.placeholder.com/32"}
                      alt=""
            ...
            {userInvite && (
              <button
                className="absolute bottom-3 right-3 flex items-center gap-1 text-xs px-2.5 py-1.5 bg-red-50 hover:bg-red-100 text-red-500 rounded-lg transition-colors border border-red-100 font-nanum-bold z-10"
                onClick={(e) => handleRejectInvite(e, item._id)}
              >
                <FiX size={12} />
                거절
              </button>
