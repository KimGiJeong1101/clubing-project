...
const HomeCard = ({ club }) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate(`/clubs/main?clubNumber=${_id}`)}
    >
      {/* 이미지 */}
      <div className="relative h-40 overflow-hidden">
        ...
        {/* 카테고리 뱃지 */}
        {mainCategory && (
          <span className="absolute top-2.5 right-2.5 bg-white/90 backdrop-blur-sm text-primary-700 text-[11px] font-nanum-bold px-2.5 py-0.5 rounded-full shadow-sm">
            {mainCategory}
          </span>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <div className="flex items-center gap-2 mb-1.5">
            <img
              src={adminImage || "https://via.placeholder.com/40"}
              alt="admin"
              className="w-7 h-7 rounded-full border-2 border-white object-cover flex-shrink-0"
            />
            <p className="text-white text-sm font-nanum-bold truncate drop-shadow-md leading-tight">
              {title || "Unknown Club"}
            </p>
          </div>
          {/* 멤버 아바타 */}
