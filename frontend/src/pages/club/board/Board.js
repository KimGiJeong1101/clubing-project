...
const Board = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showToast("게시글 저장에 실패했습니다.", false);
    }
  };

  const handleVoteSave = async () => {
    if (!title || !category || options.some((o) => !o.trim()) || !endTime) {
      showToast("모든 필드를 입력해주세요.", false);
      return;
    }
    try {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pb-20 pt-4">
      {/* Extended FAB */}
      {isMember && (
        <button onClick={() => setOpen(true)} className="fixed bottom-10 right-10 z-50 flex items-center gap-2 px-5 py-3 rounded-full bg-primary-600 hover:bg-primary-700 active:scale-95 text-white shadow-xl transition-all duration-200 text-sm font-nanum-bold" aria-label="글쓰기">
      ...
      {/* 토스트 */}
      {toast.open && <div className={`fixed top-5 left-1/2 -translate-x-1/2 z-[500] px-5 py-3 rounded-xl shadow-lg text-sm font-nanum text-white ${toast.ok ? "bg-green-500" : "bg-red-500"}`}>{toast.msg}</div>}
