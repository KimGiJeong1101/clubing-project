...
const ListPosts = () => {
  ...
  useEffect(() => { if (data) setTotalPages(data.totalPages); }, [data]);

  const items = (data?.boards || []).filter((item) => filterCat === "전체" || item.category === filterCat);

  const handleSelect = (id, cat, title) => {
    setSelectedId(id); setSelectedCat(cat); setDialogTitle(title); setOpenDialog(true);
  };

  const handleClose = () => {
    setOpenDialog(false); setSelectedId(null); setSelectedCat(""); setDialogTitle("");
  };
          ...
          <button
            ...
            className={`text-sm px-3 py-1.5 rounded-full border transition-colors font-nanum
              ${filterCat === cat
                ...
                : "border-gray-300 text-gray-600 hover:bg-primary-50 hover:border-primary-300"
              }`}
          >
            {cat}
          </button>
          ...
          <ul className="divide-y divide-gray-100">
            {items.map((item) => (
              <li
                ...
                className="flex items-center justify-between px-2 py-3.5 cursor-pointer hover:bg-primary-50 rounded-xl transition-colors"
              >
                <div className="flex-1 min-w-0 pr-3">
                  <p className="text-sm font-nanum text-gray-900 truncate">{truncate(item.title, 45)}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {item.category || "투표"}{item.endTime ? ` · 종료: ${new Date(item.endTime).toLocaleString()}` : ""}
                  </p>
                </div>
                <button
                  onClick={(e) => handleArrow(e, item._id, item.category)}
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-1 mt-6">
          <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} disabled={currentPage === 1}
            className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-600 disabled:opacity-30 hover:bg-primary-50 transition-colors text-sm">‹</button>
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[85vh] overflow-y-auto z-10">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white">
              <h2 className="text-base font-nanum-bold text-gray-900 truncate pr-4">
                {selectedCat !== "투표" ? dialogTitle : "투표"}
              </h2>
              <button onClick={handleClose} className="btn-ghost p-1 rounded-full flex-shrink-0"><FiX className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              {selectedCat === "투표"
                ? <ReadVote voteId={selectedId} title={dialogTitle} onDelete={handleClose} />
                : <Read postId={selectedId} title={dialogTitle} onClose={handleClose} />
              }
            </div>
          </div>
        </div>
