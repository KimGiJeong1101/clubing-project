...
const fetchRecommendedClubs = (email) =>
  axios.get("http://localhost:4000/clubs/home/recommend", { params: { email } }).then((r) => r.data);
  ...
  return (
    <div className="flex items-center justify-between mb-5">
      <div className="flex items-center gap-2">
        {accent && (
          <span className="w-1 h-6 rounded-full" style={{ background: "linear-gradient(to bottom, #a6836f, #dbc7b5)" }} />
        )}
        ...
        {tooltip && (
          <div className="relative">
            ...
            {tip && (
              <div className="absolute left-1/2 -translate-x-1/2 top-7 w-64 bg-gray-800 text-white text-xs rounded-xl px-3 py-2.5 z-10 shadow-xl whitespace-pre-line leading-relaxed">
                {tooltip}
              </div>
            )}
          </div>
      ...
      {onMore && (
        <button
          onClick={onMore}
          className="flex items-center gap-1 text-xs text-gray-400 hover:text-primary-600 transition-colors group"
        >
          더보기
...
const Home = () => {
  ...
  const tooltipText = email
    ? "선택한 지역 및 관심사 기준으로 추천해드립니다.\n지역 및 관심사 변경은 마이페이지 → 회원정보 → 정보수정에서 가능합니다."
    : "선택한 지역 및 관심사 기준으로 추천합니다.\n로그인 시 더 정확한 추천을 받을 수 있어요.";
    ...
    <div className="w-full min-h-screen" style={{ background: "#FAF8F5" }}>
      <div className="max-w-6xl mx-auto pb-20">
        ...
        <div className="relative w-full overflow-hidden" style={{ aspectRatio: "16/7" }}>
          ...
          <div className="absolute bottom-10 left-10 text-white">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h1 className="text-3xl md:text-4xl font-nanum-bold drop-shadow-lg mb-2">
                  {bannerTexts[curIdx].title}
                </h1>
                <p className="text-sm md:text-base text-white/80 drop-shadow">
                  {bannerTexts[curIdx].sub}
                </p>
              </motion.div>
            </AnimatePresence>
            <button
              onClick={() => navigate("/clubList")}
              className="mt-5 inline-flex items-center gap-2 bg-white/20 hover:bg-white/30 backdrop-blur-sm border border-white/40 text-white text-sm font-nanum-bold px-5 py-2 rounded-full transition-all"
            >
          ...
          <button
            onClick={prevImage}
            className="absolute left-4 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 backdrop-blur-sm border border-white/30 text-white rounded-full p-2.5 transition-all"
          >
            <FiChevronLeft className="w-5 h-5" />
          </button>
          <button
            onClick={nextImage}
            className="absolute right-4 top-1/2 -translate-y-1/2 z-10 bg-white/20 hover:bg-white/40 backdrop-blur-sm border border-white/30 text-white rounded-full p-2.5 transition-all"
          >
          ...
          <div className="absolute bottom-4 right-6 flex items-center gap-2 z-10">
            {images.map((_, i) => (
              <button
                key={i}
                onClick={() => setPage([i, i > curIdx ? 1 : -1])}
                className={`block h-1.5 rounded-full transition-all duration-300 ${
                  curIdx === i ? "w-6 bg-white" : "w-1.5 bg-white/40 hover:bg-white/60"
                }`}
              />
            ))}
          </div>
        ...
        <div className="px-4 md:px-6">
          ...
          <section className="mt-12">
            ...
            {loadingClubs ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {clubsData?.map((club, i) => <HomeCard key={i} club={club} />)}
              </div>
            )}
          </section>
          ...
          <section className="mt-14">
            ...
            <div
              ...
              style={{ height: "420px", background: "linear-gradient(135deg, #f5ede6 0%, #ece4dc 100%)", perspective: "1400px" }}
            ...
            >
              ...
              <div className="absolute inset-0 opacity-10">
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="absolute rounded-full border border-primary-400"
                    style={{ width: `${100 + i * 80}px`, height: `${100 + i * 80}px`, top: "50%", left: "50%", transform: "translate(-50%, -50%)" }} />
                ))}
              </div>
          ...
          <section className="mt-14">
            <SectionHeader
              title="추천 모임"
              onMore={() => navigate("/recommendedClubList")}
              tooltip={tooltipText}
              accent
            />
            {loadingRec ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {recommendedData.map((club, i) => <HomeCard key={i} club={club} />)}
              </div>
            ) : (
              <div className="flex flex-col items-center py-14 text-center">
                ...
                <p className="text-gray-500 text-sm">
                  {email ? "아직 추천 모임이 없어요. 관심사를 설정해보세요!" : "로그인하면 맞춤 모임을 추천해드려요"}
                </p>
                <button
                  onClick={() => navigate(email ? "/mypage" : "/login")}
                  className="mt-4 text-sm text-primary-600 hover:underline font-nanum-bold"
                >
          ...
          <section className="mt-14">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center py-14 text-center">
              ...
              <button
                onClick={() => navigate("/clubList")}
                className="mt-5 px-5 py-2 bg-primary-50 hover:bg-primary-100 text-primary-700 text-sm font-nanum-bold rounded-xl transition-colors"
              >
                모임 찾으러 가기
              </button>
            </div>
          ...
          <section className="mt-14">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center py-14 text-center">
              ...
              <button
                ...
                className="mt-5 px-5 py-2 bg-rose-50 hover:bg-rose-100 text-rose-500 text-sm font-nanum-bold rounded-xl transition-colors"
              >
                이벤트 전체보기
              </button>
            </div>
