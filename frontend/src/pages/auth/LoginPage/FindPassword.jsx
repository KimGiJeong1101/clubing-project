...
const FindPasswordPage = ({ open, onClose }) => {
  ...
  const userPassword = {
    ...
    validate: (v) => {
      ...
      if (!/^(?=.*[a-zA-Zㄱ-힝])(?=.*[\W_]).{6,}$/.test(v)) return "영문 대/소문자, 특수문자를 사용해 주세요.";
      return true;
    },
  };
  ...
  return (
    <Modal
      ...
      maxWidth="max-w-lg"
    >
      {toast.open && <div className={`mb-4 px-4 py-2.5 rounded-lg text-sm font-nanum ${toast.ok ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>{toast.msg}</div>}
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <Field label="인증번호">
          <div className="flex gap-2">
            <input className={`input-base flex-1 ${isVerified ? "bg-gray-100 text-gray-400" : ""}`} placeholder="인증번호 입력" value={verificationCode} onChange={(e) => setVerificationCode(e.target.value)} readOnly={isVerified} />
            <button
              type="button"
        ...
        <Field label="새 비밀번호">
          <div className="relative">
            <input type={showPw ? "text" : "password"} className={`input-base pr-10 ${!isVerified ? "bg-gray-100 text-gray-400" : ""}`} placeholder="새 비밀번호 (8자 이상, 특수문자 포함)" readOnly={!isVerified} {...register("newPassword", userPassword)} />
          ...
          {(errors.newPassword || passwordError) && <p className="text-xs text-red-500 mt-1">{errors.newPassword?.message || passwordError}</p>}
        </Field>
