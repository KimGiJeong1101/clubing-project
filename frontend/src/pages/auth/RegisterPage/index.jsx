...
const RegisterPage = () => {
  ...
  useEffect(() => { setValue("homeLocation.sido",    homeLocation.sido);    setValue("homeLocation.sigoon", homeLocation.sigoon); setValue("homeLocation.dong", homeLocation.dong); }, [homeLocation]);
  useEffect(() => { setValue("workplace.w_sido",     workplace.w_sido);     setValue("workplace.w_sigoon", workplace.w_sigoon);   setValue("workplace.w_dong",   workplace.w_dong); }, [workplace]);
  useEffect(() => { setValue("interestLocation.i_sido", interestLocation.i_sido); setValue("interestLocation.i_sigoon", interestLocation.i_sigoon); setValue("interestLocation.i_dong", interestLocation.i_dong); }, [interestLocation]);
  ...
  useEffect(() => {
    if (timer > 0) { const m=Math.floor(timer/60),s=timer%60; setEmailMsg(`인증번호 전송됨 [${m}:${s<10?"0":""}${s}]`); }
    else if (hasExpired) { setEmailMsg("인증번호가 만료되었습니다."); }
  }, [timer, hasExpired]);

  const handleCheckEmail = async () => {
    if (!emailValue?.trim()) { showToast("이메일을 입력해주세요.", false); return; }
    try {
      await axios.post(`${apiUrl}/users/check-email`, { email: emailValue });
      setEmailErr(""); setIsEmailChecked(true);
    } catch (e) { setEmailErr(e.response?.data?.message || "이미 사용 중인 이메일입니다."); }
  };
  ...
  const handleSendAuth = async () => {
    try {
      ...
      if (res.data.ok) {
        setCodeId(res.data.codeId); setTimer(180); setHasExpired(false);
        if (intervalId) clearInterval(intervalId);
        const id = setInterval(() => setTimer((p) => { if (p<=1){clearInterval(id);setHasExpired(true);return 0;} return p-1; }), 1000);
        setIntervalId(id);
      }
    } catch { showToast("인증 메일 발송에 실패했습니다.", false); }
  };
  ...
  const handleVerify = async () => {
    try {
      ...
      if (res.data.ok) {
        setIsVerified(true); setVerifyErr(""); setTimer(0); setHasExpired(false);
        if (intervalId) { clearInterval(intervalId); setIntervalId(null); }
        showToast("인증에 성공하였습니다.");
      } else { setVerifyErr(res.data.msg); }
    } catch { setVerifyErr("인증번호가 틀렸습니다."); }
  };
  ...
  const handleCheckNick = async () => {
    if (!nickNameValue?.trim()) { showToast("닉네임을 입력해주세요.", false); return; }
    try {
      const res = await axios.post(`${apiUrl}/users/check-nickname`, { nickName: nickNameValue });
      showToast(res.data.message); setIsNickChecked(true);
    } catch (e) { showToast(e.response?.data?.message || "서버 오류", false); }
  };
  ...
  function groupCategories(cats) {
    return cats.reduce((acc, c) => { if (!acc[c.main]) acc[c.main]=[];  acc[c.main].push(c.sub); return acc; }, {});
  }
  ...
  useEffect(() => { setValue("selectedJobs", selectedJobs); }, [selectedJobs]);
  ...
  const handleCheck = (type) => setChecks((p) => {
  ...
  /* 폼 제출 */
  const onSubmit = (data) => {
    if (!isVerified)   { showToast("이메일 인증이 완료되지 않았습니다.", false); return; }
    if (!isNickChecked){ showToast("닉네임 중복 검사를 해야 합니다.", false); return; }
    if (!data.homeLocation?.sido) { showToast("집 주소를 설정해 주세요.", false); return; }
    if (selectedJobs.length === 0){ showToast("직종을 설정해 주세요.", false); return; }
    if (selectedCategories.filter((c) => c.main).length < 3) { showToast("최소 3개의 메인 카테고리를 설정해 주세요.", false); return; }
    if (!checks.terms)  { showToast("이용약관에 동의해야 합니다.", false); return; }
    if (!checks.privacy){ showToast("개인정보 수집 및 이용에 동의해야 합니다.", false); return; }

    const body = {
      email: data.email, password: data.password, name: data.name, nickName: data.nickName,
      ...
      job: selectedJobs,
      termsAccepted: checks.terms, privacyAccepted: checks.privacy, marketingAccepted: checks.marketing,
    ...
    dispatch(registerUser(body))
      .then(() => { showToast("회원가입에 성공하셨습니다."); setTimeout(() => navigate("/"), 1000); reset(); })
  ...
  return (
    <div className="min-h-screen py-12 px-4" style={{ background: "#FAF8F5" }}>
      {/* 토스트 */}
      {toast.open && (
        <div className={`fixed top-5 left-1/2 -translate-x-1/2 z-[500] px-5 py-3 rounded-xl shadow-lg text-sm font-nanum ${toast.ok ? "bg-green-500 text-white" : "bg-red-500 text-white"}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          <Link to="/"><img src="/logo/khaki_long_h.png" alt="Clubing" className="h-14 mx-auto" /></Link>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-0">
            ...
            <Field label="이메일" required error={emailErr || errors.email?.message}>
              <div className="flex gap-2">
                ...
                />
                {!isEmailChecked
                  ? <button type="button" onClick={handleCheckEmail} className="btn-primary whitespace-nowrap px-4">중복검사</button>
                  : (
                    <div className="flex flex-col gap-1">
                      <button type="button" onClick={() => setIsEmailChecked(false)} className="btn-outline text-xs px-3 py-1">수정</button>
                      <button type="button" onClick={handleSendAuth} className="btn-primary text-xs px-3 py-1">인증하기</button>
                    </div>
                  )
                }
              </div>
            ...
            <Field label="비밀번호" required error={errors.password?.message}>
              <div className="relative">
                <input type={showPw ? "text" : "password"} placeholder="8자 이상, 특수문자 포함" className="input-base pr-10"
                  {...register("password", { required: "필수 필드입니다.", minLength: { value: 8, message: "최소 8자입니다." }, validate: (v) => /^(?=.*[a-zA-Zㄱ-힝])(?=.*[\W_]).{6,}$/.test(v) || "영문 + 특수문자를 사용해 주세요." })} />
            ...
            <Field label="비밀번호 확인" required error={errors.passwordCheck?.message}>
              <div className="relative">
                <input type={showPw2 ? "text" : "password"} placeholder="비밀번호 재입력" className="input-base pr-10"
                  {...register("passwordCheck", { required: "필수 필드입니다.", validate: (v) => v === watch("password") || "비밀번호가 일치하지 않습니다." })} />
            ...
            <Field label="이름" required error={errors.name?.message}>
              <input className="input-base" placeholder="이름" {...register("name", { required: "필수 필드입니다.",
                validate: (v) => {
            ...
            <Field label="닉네임" required error={errors.nickName?.message}>
              <div className="flex gap-2">
                <input className={`input-base flex-1 ${isNickChecked ? "bg-gray-100 text-gray-400" : ""}`} placeholder="닉네임" readOnly={isNickChecked}
                  {...register("nickName", { required: "필수 필드입니다.", maxLength: { value: 20, message: "최대 20자입니다." } })} />
                {isNickChecked
                  ? <button type="button" onClick={() => setIsNickChecked(false)} className="btn-outline px-4">수정</button>
                  : <button type="button" onClick={handleCheckNick} className="btn-primary px-4 whitespace-nowrap">중복검사</button>
                }
              </div>
            </Field>
            ...
            <Field label="생년월일" required>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <Controller key={name} name={name} control={control} rules={rules} render={({ field }) => (
            ...
            <Field label="성별" required error={errors.gender?.message}>
              <Controller name="gender" control={control} rules={{ required: "성별을 선택해 주세요." }} render={({ field }) => (
                <div className="flex gap-3">
                  {[{ v: "남성", label: "남자", active: "bg-blue-600 border-blue-600 text-white", inactive: "border-blue-300 text-blue-600 hover:bg-blue-50" },
                    { v: "여성", label: "여자", active: "bg-pink-500 border-pink-500 text-white", inactive: "border-pink-300 text-pink-500 hover:bg-pink-50" }].map(({ v, label, active, inactive }) => (
                    <button key={v} type="button" onClick={() => field.onChange(v)}
                      className={`flex-1 py-2.5 rounded-lg border-2 font-nanum-bold text-sm transition-colors ${field.value===v ? active : inactive}`}>
                      {label}
                    </button>
                  ))}
                </div>
            ...
            <Field label="전화번호" required error={errors.phone?.message}>
              <input className="input-base" placeholder="010-0000-0000"
                {...register("phone", { required: "필수 필드입니다.", pattern: { value: /^\d{3}-\d{4}-\d{4}$/, message: "010-0000-0000 형식으로 입력해 주세요." } })}
                onChange={(e) => setValue("phone", formatPhone(e.target.value), { shouldValidate: true })}
                value={watch("phone")} maxLength={13} />
            </Field>

            {/* 주소 */}
            <div className="mb-5 space-y-4">
              <div>
                <Label required>집주소 <span className="font-normal text-gray-400 normal-case">(읍면동 단위)</span></Label>
                <HomeSearch
              ...
              <div>
                <Label>직장주소 <span className="font-normal text-gray-400 normal-case">(읍면동 단위)</span></Label>
                <WorkplaceSearch
                  setWorkplaceSido={(v) => setWorkplace((p) => ({ ...p, w_sido: v }))}
                  setWorkplaceSigoon={(v) => setWorkplace((p) => ({ ...p, w_sigoon: v }))}
                  setWorkplaceDong={(v) => setWorkplace((p) => ({ ...p, w_dong: v }))} />
              </div>
              <div>
                <Label>관심지역 <span className="font-normal text-gray-400 normal-case">(읍면동 단위)</span></Label>
                <InterestSearch
                  setInterestSido={(v) => setInterestLocation((p) => ({ ...p, i_sido: v }))}
                  setInterestSigoon={(v) => setInterestLocation((p) => ({ ...p, i_sigoon: v }))}
                  setInterestDong={(v) => setInterestLocation((p) => ({ ...p, i_dong: v }))} />
              </div>
            </div>

            {/* 직종 */}
            <Field label="직종 (최대 3개)">
              <button type="button" onClick={() => setPopup((p) => ({ ...p, job: true }))} className="btn-outline mb-2">직종 선택</button>
            ...
            <Field label="관심사 카테고리 (3개 이상)">
              <button type="button" onClick={() => setPopup((p) => ({ ...p, category: true }))} className="btn-outline mb-2">카테고리 선택</button>
            ...
            <div className="mb-6 mt-6">
              ...
              {[
                ...
                <div key={key} className="flex items-center justify-between py-2 border-b border-gray-100">
                  ...
                  <button type="button" onClick={() => setPopup((p) => ({ ...p, [key]: true }))} className="text-xs text-primary-600 hover:underline">전체보기</button>
                </div>
              ))}
            </div>
