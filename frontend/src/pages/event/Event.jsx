...
const Event = () => {
    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-[1300] bg-black/50 flex items-center justify-center" onClick={onClose}>
          <motion.div initial={{ opacity: 0, y: 40, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.96 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} onClick={(e) => e.stopPropagation()} className="relative bg-white rounded-2xl shadow-2xl p-8 w-full max-h-[85vh] overflow-y-auto mx-4" style={{ maxWidth }}>
            {children}
          </motion.div>
        </div>
      )}
  ...
  return (
    <div className="w-full min-h-screen" style={{ background: "#FAF8F5" }}>
            ...
            {[
              ...
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-nanum-bold transition-all duration-200
                  ${filter === key ? "bg-primary-600 text-white shadow-sm" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
              >
                {label}
                <span className="ml-1.5 text-xs opacity-70">({events.filter((e) => (key === "ongoing" ? !e.endTime || new Date(e.endTime) > new Date() : e.endTime && new Date(e.endTime) <= new Date())).length})</span>
              </button>
            ))}
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
            {filteredEvents.map((event) => (
              <EventListCard key={event._id} event={event} onEdit={handleEditEvent} onDelete={handleDeleteEvent} onImageClick={() => handleOpenDetail(event._id)} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center py-20 text-center">
            ...
            <p className="font-nanum-bold text-gray-700 mb-1">{filter === "ongoing" ? "진행 중인 이벤트가 없어요" : "종료된 이벤트가 없어요"}</p>
      ...
      {isAdmin && (
        <>
          <button onClick={() => setOpenCreate(true)} className="fixed bottom-10 right-10 z-50 w-14 h-14 rounded-full bg-primary-600 hover:bg-primary-700 active:scale-95 text-white shadow-xl flex items-center justify-center transition-all duration-200" aria-label="이벤트 만들기">
