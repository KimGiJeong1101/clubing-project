...
const MessageInput = ({ message, setMessage, handleSendMessage, handleKeyPress }) => {
  ...
  const handleEmojiClick = (emoji) => {
    if (emoji && messageInputRef.current) {
      ...
      setTimeout(() => {
        messageInputRef.current.selectionStart = messageInputRef.current.selectionEnd =
    ...
    <div className="flex items-center gap-2 px-3 py-2.5 bg-white border-t border-gray-100 rounded-b-2xl">
      ...
      <div className="relative flex-1">
        <input
          ...
          className="w-full bg-gray-50 border border-gray-200 rounded-full px-4 py-2.5 text-sm outline-none focus:border-primary-300 focus:ring-1 focus:ring-primary-100 transition-all"
        />
        {showPicker && (
      ...
      <button
        ...
        className="flex-shrink-0 flex items-center justify-center w-10 h-10 bg-primary-600 hover:bg-primary-700 rounded-full transition-colors shadow-sm"
        aria-label="메시지 전송"
      >
