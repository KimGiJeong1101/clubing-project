            ...
            <div className="flex flex-col items-center mb-6 pt-4">
              <div className="relative mb-3">
                <img
                  ...
                  onClick={() => {
                    setImageSrc(user?.profilePic?.originalImage || "");
                    setOpen(true);
                  }}
                />
                <button onClick={() => setShowPopover(true)} className="absolute bottom-0 right-0 w-7 h-7 bg-primary-600 rounded-full shadow-md flex items-center justify-center hover:bg-primary-700 transition-colors">
                ...
                {showPopover && (
                  <>
                    ...
                    <div className="absolute left-8 bottom-0 z-50 bg-white border border-gray-200 rounded-xl p-1.5 shadow-xl flex flex-col gap-0.5 min-w-[120px]">
                      <button onClick={handleEditClick} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-50 text-sm text-gray-700 transition-colors">
                        <FiEdit className="w-3.5 h-3.5" /> 수정하기
                      </button>
                      <button onClick={handleDeleteClick} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-red-50 text-sm text-red-500 transition-colors">
              ...
              <div className="flex items-center gap-1.5 mt-3 w-full justify-center">
                <p className="text-sm text-gray-600 text-center">{user?.profilePic?.introduction || "소개글을 작성해보세요"}</p>
                <button
                  onClick={() => {
                    setNewIntroduction(user?.profilePic?.introduction || "");
                    setOpenModal(true);
                  }}
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {!showAll && remainingCount > 0 && (
                    <button onClick={() => setShowAll(true)} className="px-2.5 py-0.5 bg-gray-100 text-gray-500 text-xs font-nanum-bold rounded-full hover:bg-gray-200 transition-colors">
                      +{remainingCount}
                    </button>
                  )}
                  {showAll && remainingCount > 0 && (
                    <button onClick={() => setShowAll(false)} className="px-2.5 py-0.5 bg-gray-100 text-gray-500 text-xs font-nanum-bold rounded-full hover:bg-gray-200 transition-colors">
                      접기
                    </button>
                  )}
                </div>
                ...
                <div key={route.path} className="relative">
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-nanum-bold transition-all duration-200
                      ${selectedSection === route.path ? "bg-primary-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"}`}
                  >
                    {route.name}
                  </button>
                  {route.path === "mymessage" && unreadCount > 0 && <span className="absolute top-1/2 -translate-y-1/2 right-3 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center">{unreadCount}</span>}
                </div>
              ))}
            </nav>
          </div>
        ...
        <div className="fixed inset-0 z-[300] bg-black/50 flex items-center justify-center">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-[380px] relative mx-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setOpenModal(false)} className="w-7 h-7 rounded-full hover:bg-gray-100 flex items-center justify-center transition-colors">
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={handleSaveIntroduction} className="flex-1 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-nanum-bold transition-colors">
                저장하기
              </button>
              <button onClick={() => setOpenModal(false)} className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl text-sm font-nanum-bold transition-colors">
                취소
              </button>
            </div>
          </div>
      ...
      {open && (
        <div className="fixed inset-0 z-[300] bg-black/80 flex items-center justify-center" onClick={() => setOpen(false)}>
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setOpen(false)} className="absolute top-3 right-3 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center hover:bg-white transition-colors shadow">
